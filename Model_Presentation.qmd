...
subtitle: "Credit Risk Modelling Challenge"
...
format: 
  revealjs:
    ...
    footer: "https://iamolumide.quarto.pub/credit-risk-model-presentation/"
    incremental: true
    smaller: true
    scrollable: true
    code-fold: show
...
## Problem Identification {transition="fade-in slide-out"}
...
umap1 <- crd_rec_trained %>% 
  step_umap(all_numeric_predictors(), num_comp = 4) %>% 
  plot_validation_results() +
  ggtitle("UMAP")

umap2 <- crd_rec_trained %>% 
...
roc_auc(prob_preds, truth = default, estimate = `.pred_no`) %>% datatable()
```
...
> From the variable importance plot, `checking_balance` is the least important feature in the final model whereas the `job`, `amount`, and `existing_credits` features are the top 3 most important among the selected features in the final model.
